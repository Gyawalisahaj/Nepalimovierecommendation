...
export default function MovieCard({ movie }: { movie: Movie }) {
  return (
    <div className="shadow-md p-4  rounded-2xl border-8 border-[#6D271B]  bg-[#F2ECE3]">
          ...
          movie.video_url ? (
            <a href={movie.video_url} target="_blank" rel="noopener noreferrer">
              <img
                src={movie.image_url}
                alt={movie.title}
                className="mt-3 rounded-2xl h-80 w-3xl mr-10"
              />
            </a>
          ) : (
            <img
              src={movie.image_url}
              alt={movie.title}
              className="mt-3 rounded-md w-3xl opacity-60"
            />
          )
        ) : null}
        <div className="bg-orange-200 rounded-2xl text-orange-800 ml-10  border">
